...
class SATRules(object):
    def __init__(self, data, options):
        ...
        # samples divided into classes：将samps初始化为字典，其中key是特征值索引，value是空列表。特征值索引是通过fvmap对象在self.data的最后一列中查找特征v得到的
        # self.data.names[-1]：data中最后一列的名称
        # self.data.feats[-1]：data中最后一列的特征值
        # sorted(self.data.feats[-1])：将特征值按升序排列
        ...
        # binarize dataset if necessary ： one hot编码
        self.binarize()
    ...
    def reset_idpool(self):
        ...
        self.idpool = IDPool(start_from=1) #IDPool的定义在何处？
    ...
    def compute(self):
        ...
        if self.options.to_compute == 'best':
            ...
            computed = len(self.data.feats[-1]) - len(to_compute) #所有class的数量-要计算的class数量
            self.labels = [self.data.fvmap.dir[self.data.names[-1], c] for c in to_compute]

        while True:
            for label in self.labels:
                if self.covrs[label]: #返回label对应的特征值
                    continue
    ...
    def encode(self, label, nof_terms=1):
        ...
        # constraint 6 每个rule必须至少有一个特征r中的literal
